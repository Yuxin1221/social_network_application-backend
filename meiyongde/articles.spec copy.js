...
describe('test validate post/article',async()=>{
    it('post article', async (done) => {
        ...
        .then(res => {
            ...
            expect(res.posts[0].title).toEqual(testPost.title);
            expect(res.posts[0].content).toEqual(testPost.content);
            done();
        })
        .catch(err => {done(new Error(err))});
    })
    ...
    it('validate GET /articles/id -post', async (done) => {
        return fetch(url('/articles' + '/' + postId), {
            method: 'GET',
            headers: { 'Content-Type': 'application/json', 'cookie': cookie  },
        })
        .then(res => res.json())
